/* ==========================================================
   ...
   function openEditModal(id) {
     ...
     // ❌ ลบการดึงค่า editRating ออกจากตรงนี้แล้ว
   ...
   document.getElementById('editProductForm').addEventListener('submit', async function (e) {
     ...
     // ❌ ลบการดึงตัวแปร rating ออกจากตรงนี้แล้ว
     ...
     const payload = {
       ...
       // ❌ ลบการส่งค่า rating ใน payload ออกจากตรงนี้แล้ว
     };
   ...
   /* ----------------------------------------------------------
      Initialize (ระบบตรวจสอบสิทธิ์ + ป๊อปอัปแจ้งเตือน)
      ---------------------------------------------------------- */
   document.addEventListener('DOMContentLoaded', () => {
       ...
       if (isAuthenticated !== 'true' || userRole !== 'admin') {
           ...
           // 1. สั่งเบลอเนื้อหาที่อยู่หลังป๊อปอัป
           if (pageContainer) {
               pageContainer.style.filter = 'blur(10px)';
               pageContainer.style.pointerEvents = 'none'; // ป้องกันการแอบคลิก
           }
   
           // 2. เรียกใช้งาน SweetAlert2 ให้เด้งเตือน
           if (typeof Swal !== 'undefined') {
               Swal.fire({
                   icon: 'error',
                   title: 'Access Denied',
                   text: 'Administrator privileges required. Please login first.',
               ...
               // กรณีฉุกเฉินถ้าเน็ตช้าจนโหลด SweetAlert2 ไม่ทัน
               alert("Access Denied. Administrator privileges required.");
               window.location.href = 'login.html';
           }
   
       } else {
           // ถ้าล็อกอินแล้ว (ผ่านฉลุย) ให้โหลดข้อมูลสินค้าขึ้นมาโชว์
           if (pageContainer) {
